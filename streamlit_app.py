...
if _url_cfg is not None:
    ...
    if st.session_state.get("_cfg_hash") != _cfg_hash:
        ...
        st.session_state["w_dome_radius"] = float(_url_cfg.get("r", 1.0))
...
with col1:
    N_points = st.number_input("Numer of total Speakers", min_value=1, max_value=1000, value=23, step=1, key="w_n_points")
with col2:
...
with col3:
    dome_radius = st.number_input("Dome Radius (m)", min_value=0.1, max_value=500.0, value=1.0, step=0.5, key="w_dome_radius")
...
for i in range(len(default_theta_vals)):
    with st.expander(f"Ring {i+1}", expanded=False):
        col_ring, col_truss = st.columns(2)
...
col_rw, col_rl, col_rh, col_rlh = st.columns(4)
with col_rw:
...
with col_rlh:
    listener_height = st.number_input("Listener Height (m)", min_value=0.0, max_value=50.0, value=1.2, step=0.1)
